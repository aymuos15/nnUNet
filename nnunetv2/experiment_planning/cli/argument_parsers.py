...
def add_dataset_arguments(parser: argparse.ArgumentParser) -> None:
    """Add common dataset ID arguments to parser."""
...
def add_fingerprint_arguments(parser: argparse.ArgumentParser) -> None:
    """Add fingerprint extraction specific arguments to parser."""
...
def add_planning_arguments(parser: argparse.ArgumentParser) -> None:
    """Add experiment planning specific arguments to parser."""
...
def add_preprocessing_arguments(parser: argparse.ArgumentParser) -> None:
    """Add preprocessing specific arguments to parser."""
...
def add_common_arguments(parser: argparse.ArgumentParser) -> None:
    """Add arguments common to all entry points."""
    parser.add_argument('--verbose', required=False, action='store_true',
                        help='Set this to print a lot of stuff. Useful for debugging. Will disable progress bar! '
                             'Recommended for cluster environments')


def create_fingerprint_parser() -> argparse.ArgumentParser:
    """Create parser for fingerprint extraction entry point."""
    parser = argparse.ArgumentParser()
    add_dataset_arguments(parser)
    add_fingerprint_arguments(parser)
    add_common_arguments(parser)
    return parser


def create_planning_parser() -> argparse.ArgumentParser:
    """Create parser for experiment planning entry point."""
    parser = argparse.ArgumentParser()
    add_dataset_arguments(parser)
...
def create_preprocessing_parser() -> argparse.ArgumentParser:
    """Create parser for preprocessing entry point."""
    parser = argparse.ArgumentParser()
    add_dataset_arguments(parser)
...
def create_combined_parser() -> argparse.ArgumentParser:
    """Create parser for combined plan and preprocess entry point."""
    parser = argparse.ArgumentParser()
    add_dataset_arguments(parser)

    # Fingerprint extraction args (with modified naming for combined use)
    parser.add_argument('-fpe', type=str, required=False, default='DatasetFingerprintExtractor',
                        help='[OPTIONAL] Name of the Dataset Fingerprint Extractor class that should be used. Default is '
                             '\'DatasetFingerprintExtractor\'.')
...
def get_default_num_processes(configurations: list) -> list:
    """Get default number of processes for given configurations."""
    default_np = {"2d": 8, "3d_fullres": 4, "3d_lowres": 8}
    return [default_np[c] if c in default_np.keys() else 4 for c in configurations]