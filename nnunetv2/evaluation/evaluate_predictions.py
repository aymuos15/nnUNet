...
def panoptic_dice(mask_ref: np.ndarray, mask_pred: np.ndarray, ignore_mask: np.ndarray = None):
    ...
    print('Num GT CC:', torch.unique(gt_cc).size(0))
    ...
    for pred_item, pred_mask in pred_masks.items():
        for gt_item, gt_mask in gt_masks.items():
            if torch.any(torch.logical_and(pred_mask, gt_mask)):
                ...
                dice_scores[(pred_item, gt_item)] = torch_dice(pred_mask, gt_mask)
...
def compute_metrics(reference_file: str, prediction_file: str, image_reader_writer: BaseReaderWriter,
    ...
    for r in labels_or_regions:
        ...
        #! ###################Choose Instance DSC Style ######################
        # lesion_dice, _ = cluster_dice(mask_ref, mask_pred, ignore_mask)
        lesion_dice, _ = panoptic_dice(mask_ref, mask_pred, ignore_mask)
        #! ###################################################################
        if tp + fp + fn == 0:
