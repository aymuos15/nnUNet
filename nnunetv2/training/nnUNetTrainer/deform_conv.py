...
#         self.offset_conv_kernel = nn.Conv3d(in_channels, N * 3, kernel_size=self.kernel_size, 
...
#             x = x.to(dtype=self.offset_conv_kernel.weight.dtype, device=self.offset_conv_kernel.weight.device)
#             offset = self.offset_conv_kernel(x)
#         else:
#             b, c, h, w, d = x.size()
#             offset = torch.zeros(b, 3 * self.kernel_size[0] * self.kernel_size[1] * self.kernel_size[2], h, w, d).to(x)
...
#         mask = torch.cat([
#             p[..., :N].lt(self.padding[0]) + p[..., :N].gt(x.size(2) - 1 - self.padding[0]),
#             p[..., N:2 * N].lt(self.padding[1]) + p[..., N:2 * N].gt(x.size(3) - 1 - self.padding[1]),
#             p[..., 2 * N:].lt(self.padding[2]) + p[..., 2 * N:].gt(x.size(4) - 1 - self.padding[2]),
#         ], dim=-1).type_as(p)
#         mask = mask.detach()
...
#         x_offset = self.conv_kernel(x_offset)

#         return x_offset

#     def _get_p(self, offset, dtype):
#         kernel_size = self.kernel_size if isinstance(self.kernel_size, tuple) else (self.kernel_size,) * 3
#         ranges = [range(-(size - 1) // 2, (size - 1) // 2 + 1) for size in kernel_size]
#         grids = torch.meshgrid(*ranges, indexing='ij')
#         p_n = torch.stack([g.flatten() for g in grids], dim=1).to(dtype)
#         return p_n
...
#         for i in range(N):
#             q_i = q[..., i]
#             x_q.append(self._get_x(x, q_i))
#         x_q = torch.cat(x_q, dim=1)
#         return x_q

#     def _get_x(self, x, q):
#         x = x.permute(0, 2, 3, 4, 1)
#         x = x.contiguous().view(-1, x.size(4))
#         x = torch.index_select(x, 0, q.view(-1))
#         x = x.view(q.size() + (-1,))
#         x = x.permute(0, 4, 1, 2, 3)
#         return x

#     def zero_padding(self, x):
#         padding = (self.padding[0], self.padding[0], self.padding[1], self.padding[1], self.padding[2], self.padding[2])
...
# from torch.nn import ModuleList, Conv3d
# from nnunetv2.training.nnUNetTrainer.deform_conv import DeformConv3d

# #! ################### Deformable Convolutional Layer ###################

# def replace_conv_layers_with_deformconv(model):
#     changed_layers = []
#     conv3d_count_before = 0
#     deformconv3d_count_before = 0

#     def count_layers(module):
#         nonlocal conv3d_count_before, deformconv3d_count_before
#         for child in module.children():
#             if isinstance(child, nn.Conv3d):
#                 conv3d_count_before += 1
#             elif isinstance(child, DeformConv3d):
#                 deformconv3d_count_before += 1
#             if isinstance(child, (nn.Sequential, nn.ModuleList, nn.ModuleDict)):
#                 count_layers(child)

#     count_layers(model)
#     print(f"Before replacement - Conv3d layers: {conv3d_count_before}, DeformConv3d layers: {deformconv3d_count_before}")

#     def replace_conv_in_module(module):
#         nonlocal changed_layers
#         conv_count = 0
#         total_convs = []

#         def count_conv_layers(mod):
#             nonlocal conv_count
#             for name, child in mod.named_children():
#                 if isinstance(child, nn.Conv3d):
#                     conv_count += 1
#                     total_convs.append((name, child))
#                 elif isinstance(child, (nn.Sequential, nn.ModuleList, nn.ModuleDict)):
#                     count_conv_layers(child)

#         count_conv_layers(module)
#         print(f"Total Conv3d layers found in module: {conv_count}")

#         def replace_convs(mod):
#             nonlocal changed_layers
#             conv_counter = 0
#             for name, child in mod.named_children():
#                 if isinstance(child, nn.Conv3d):
#                     print(f"Found Conv3d layer: {name}")
#                     if conv_counter < (conv_count - 2):  # Replace all but the last 2 Conv3d layers
#                         print(f"Replacing Conv3d in {name}")
#                         changed_layers.append(name)
#                         new_layer = DeformConv3d(
#                             child.in_channels,
#                             child.out_channels,
...
#                     else:
#                         print(f"Skipping replacement for {name} (one of the last two Conv3d layers)")
#                     conv_counter += 1
#                 elif isinstance(child, (nn.Sequential, nn.ModuleList, nn.ModuleDict)):
#                     replace_convs(child)

#         replace_convs(module)

#     # Replace conv layers in encoder
#     if hasattr(model, 'encoder'):
#         print("Replacing Conv3d layers in encoder...")
#         replace_conv_in_module(model.encoder)
#     else:
#         raise AttributeError("Model does not have an 'encoder' attribute")

#     print("Finished replacement process.")
    
#     # Print out the changed layers
#     if changed_layers:
#         print("The following layers were changed:")
#         for layer in changed_layers:
#             print(layer)
#     else:
#         print("No Conv3d layers were replaced.")

#     # Count layers after replacement
#     conv3d_count_after = 0
#     deformconv3d_count_after = 0
#     count_layers(model)
#     print(f"After replacement - Conv3d layers: {conv3d_count_after}, DeformConv3d layers: {deformconv3d_count_after}")

#     return model

# # Usage example:
# # model = YourModelClass()
# # model = replace_conv_layers_with_deformconv(model)
...
#         print(self.network)
#         #! ####################### DEFORMABLE ##########################
#         self.network = replace_conv_layers_with_deformconv(self.network)
#         #! #############################################################
#         print(self.network)
